"""
LLaVA-NeXT model adapter for ShortV, providing concrete implementations for the ShortV interface.
"""
import os
import time
import torch
...
class LlamaNextPatcher(BasePatcher):
    """Patcher for LLaMA-like models in LLaVA-NeXT."""
    def _get_layers(self) -> torch.nn.ModuleList:
        """Accesses the decoder layers of a LLaMA model."""
        return self.model.model.layers

class LLaVANeXTShortV(ShortV):
    """
    LLaVA-NeXT-specific ShortV implementation.

    This adapter enables visual token skipping for LLaVA-NeXT models by dynamically
    ...
    def __init__(self, model_name: str, n_skip_layers: Optional[int] = None,
        ...
        """
        Initialize LLaVA-NeXT ShortV, using pre-computed configs if available.
        """
        super().__init__(model_name, n_skip_layers, skip_layers, **kwargs)
        self._original_llava_next_forward = None
        ...
        self.patcher = LlamaNextPatcher(self.model.language_model)
    ...
    def _create_patched_llava_next_forward(self):
        """Creates a patched forward method for LlavaNextForConditionalGeneration that preserves the original signature."""
    ...
    def enable_monkey_patching(self):
        """Enables monkey patching for LLaVA-NeXT models."""
        if self._patching_enabled:
            return

        # 1. Patch LlavaNextForConditionalGeneration.forward to pass image_token_indices
        if not self._original_llava_next_forward:
            self._original_llava_next_forward = self.model.forward
            self.model.forward = self._create_patched_llava_next_forward()
            logger.info("Patched LlavaNextForConditionalGeneration.forward to pass image token indices.")
    ...
    def disable_monkey_patching(self):
        ...
        # Restore original LLaVA-NeXT forward if it was patched
        if self._original_llava_next_forward:
            self.model.forward = self._original_llava_next_forward
            self._original_llava_next_forward = None
    ...
    def calculate_lc_scores(self, ) -> Dict[str, Any]:
        """
        Calculates Layer Contribution (LC) scores for LLaVA-NeXT models using KL-Divergence.
...
ModelRegistry.register_shortv("llava_next", LLaVANeXTShortV)